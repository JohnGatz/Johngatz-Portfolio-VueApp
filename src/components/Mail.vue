...
<template>
  <interact
    ...
    :style="style"
    @dragmove="dragmove"
    :class="{ fullscreen: $store.getters.isFullscreenMail }"
  >
    <iframe
      name="hidden_iframe"
      id="hidden_iframe"
      style="display:none;"
      onload="if(this.submitted)  {window.location='https://bigsurmacos.netlify.app/';}"
    ></iframe>
    <form
      v-on:submit="sendEmail"
      action="https://docs.google.com/forms/u/1/d/e/1FAIpQLSd4-7nMPrD4n-StnK79_2GTCTzmtPeoa7u3POx0jV3XD4Nsbg/formResponse"
      class="window"
      id="container"
      :class="{
        fullscreen: $store.getters.isFullscreenMail,
        close: !$store.getters.isShownMail,
      }"
    ...
    >
      <div
        class="top-bar"
        id="top-bar"
        v-on:dblclick="$store.commit('toggleFullscreenMail')"
        :class="
          $store.getters.activeWindow == 'Mail'
            ? 'top-bar'
            : 'top-bar-deactivated'
        "
      >
        ...
        <div class="triple-button">
          <div class="button-hide" v-on:click="minimizeMail">
            <span
          ...
          <div
            class="button-expand"
            v-on:click="$store.commit('toggleFullscreenMail')"
          >
            <span
